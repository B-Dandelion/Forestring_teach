...
begin
  ...
  loop
    begin
      ...
      -- A withdrawn legacy identity contributes history only strictly before
      -- its effective withdrawal date. Never import current/future lessons.
      ...
      -- Missing-history recovery only: never alter or duplicate an existing
      -- lesson, regardless of its current cancellation state.
      if exists (
        select 1
...
create or replace function private.import_merged_firebase_history_text_atomic(p_payload text)
returns jsonb
language plpgsql
security definer
set search_path = ''
...
begin
  ...
  v_result := private.import_merged_firebase_history_text(p_payload);
  if coalesce((v_result->>'failed')::integer, 0) > 0 then
    raise exception using errcode='P0001', message='FORESTRING_MERGED_HISTORY_BATCH_ROLLED_BACK', detail=v_result::text;
  end if;
  return v_result;
end;
$function$;
...
revoke all on function private.import_merged_firebase_history_text_atomic(text) from public, anon, authenticated;