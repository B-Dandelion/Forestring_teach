-- ============================================================
-- Forestring v3
-- Master creates manager account
--
-- canTeach = false
    ...
    loop
      ...
      if v_weekday not between 1 and 7 then
        raise exception using
          errcode = 'P0001',
          message =
            'FORESTRING_INVALID_WORK_WEEKDAY';
      end if;
      ...
      if v_start_time >= v_end_time then
        raise exception using
          errcode = 'P0001',
          message =
            'FORESTRING_INVALID_WORK_TIME_RANGE';
      end if;


      if (
        extract(
          minute from v_start_time
        )::integer % 15 <> 0
        or
        extract(
          minute from v_end_time
        )::integer % 15 <> 0
      ) then
        raise exception using
          errcode = 'P0001',
