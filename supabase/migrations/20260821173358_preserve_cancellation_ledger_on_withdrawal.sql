-- ============================================================
-- Forestring v3
-- Preserve cancellation ledger when withdrawal hard-deletes
-- future lessons.
--
-- lesson_cancellation_events remains immutable.
-- lesson_id remains the historical UUID but is no longer an FK,
-- because a withdrawn student's future lesson may be removed.
-- ============================================================

alter table public.lesson_cancellation_events
drop constraint if exists
  lesson_cancellation_events_lesson_id_fkey;


create index if not exists
  lesson_cancellation_events_lesson_idx
...
comment on column
  public.lesson_cancellation_events.lesson_id
is
  'Historical lesson UUID. Integrity is validated when the cancellation event is inserted. The referenced lesson may later be physically removed by student withdrawal while this immutable cancellation ledger remains preserved.';
...
declare
  ...
  v_preserved_cancellation_count integer := 0;
...
begin
  ...
  into v_preserved_cancellation_count
  ...
  -- Cancellation events are immutable historical records.
  -- Their lesson_id remains as the deleted lesson's UUID.
  --
  ...
  values (
    ...
    jsonb_build_object(
      ...
      'preservedCancellationEventCount',
        v_preserved_cancellation_count,
...
comment on function public.finalize_student_withdrawal(
  ...
  'Finalizes a scheduled student withdrawal once its KST effective date has arrived. Actual lessons whose starts_at is on/after the withdrawal boundary are hard-deleted while immutable cancellation events are preserved and legacy-credit rows are removed; historical lessons are preserved, remaining active rights are revoked, assignment/recurring schedules are closed, and the student profile is deactivated atomically.';