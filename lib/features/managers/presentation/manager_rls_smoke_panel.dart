...
class ManagerRlsSmokePanel extends StatefulWidget {
  ...
  @override
  State<ManagerRlsSmokePanel> createState() =>
      _ManagerRlsSmokePanelState();
}

class _ManagerRlsSmokePanelState
    extends State<ManagerRlsSmokePanel> {
  ...
  Future<void> _runTest() async {
    ...
    try {
      ...
      final teacherRaw = await client
          .from('teachers')
          .select(
            'id, teaching_enabled',
          );

      final studentRaw = await client
          .from('students')
          .select(
            'id',
          );

      final lessonRaw = await client
          .from('lessons')
          .select(
            'id',
          );
