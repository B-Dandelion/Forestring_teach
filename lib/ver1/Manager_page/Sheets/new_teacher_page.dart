...
import 'package:forestring_teacher_2/ver1/Manager_page/Sheets/custom_text_field.dart';
...
class _new_teacher_page extends State<new_teacher_page>{
  ...
  void onSavePressed(BuildContext context) async{
    if(formKey.currentState!.validate()){
      ...
      final String NewteacherID = DateFormat('yyMMdd').format(DateTime.now()) + password;

      await FirebaseFirestore.instance.collection('teacher').doc(NewteacherID).set({'id':NewteacherID, 'name': name, 'pw': password, 'students' : []});
      ...
      await FirebaseFirestore.instance.collection('teacher').doc(NewteacherID).collection('BanTime').doc(DateFormat('yyyyMMdd').format(DateTime.now().subtract(const Duration(days: 300))))
      .set({'0': DateTime.now().subtract(const Duration(days: 300)),'1': DateTime.now().subtract(const Duration(days: 300))});
    }
