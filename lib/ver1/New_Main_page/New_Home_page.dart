...
class _New_Home_page extends State<New_Home_page> {
  ...
  List<Lesson> _getEvents(DateTime day) {
    //일정 정보를 20240910 : [] 형식으로 저장해주는 함수임.
    ...
    for (int i = 0; i<LessonList.length; i++){
      if(DateFormat('yyyyMMdd').format(DateTime.parse(LessonList[i].time.toString())) == Dayformat) {
        if (LessonList[i].isValid == true) {
          ...
          if (events != null &&
              events.containsKey(DateTime.utc(day.year, day.month, day.day))) {
            events[DateTime.utc(day.year, day.month, day.day)].add(
                LessonList[i]);
            EVENTS.add(events);
          } else {
            events = {
                                  ...
                                  itemBuilder: (context, index){
                                    ...
                                    return Padding(padding: const EdgeInsets.only(bottom: 8, left: 8, right: 8),
                                        child: LessonCard_main(startTime: tmp, month: tmp.month, lessonID: EVENTS.id,
                                            date: tmp.day, student: StudentName[EVENTS.id.substring(0,12)]!, teacher: User.name));
                                  }
                              )
                          )
                      ),
